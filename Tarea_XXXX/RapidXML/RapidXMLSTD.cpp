...
/**Saves and XML objects to a file
* @param xsd_top - A xml document object
* @param xsd_bottom - A xml document object
* @param filePath - The path to an xml file
* @returns True if success*/
const bool SaveXML2(const XMLDocument& xsd_top, const XMLDocument& xsd_bottom, const std::string& filePath)
{
	// Save to file
	std::ofstream file_stored(filePath);
	...
	file_stored << xml_head << std::endl;
	file_stored << xml_content << std::endl;
	file_stored << xsd_top;
	file_stored << xsd_bottom;
	file_stored << xml_closure << std::endl;
