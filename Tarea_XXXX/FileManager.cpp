...
std::optional<std::string> FileManager::LoadFromMicroSD(std::string_view filename)
{
	// TODO Loading from uSD, we need to implement that in stm32.
	std::ifstream file(std::string(filename), std::ios::in | std::ios::binary);
	if (!file.is_open()) {
		LOG("Error: Could not open file %s for reading.\n", filename.data());
		return std::nullopt;
	}
...
void FileManager::SaveToMicroSD(std::string_view filename, std::string_view data)
{
	// TODO Store to uSD, we need to implement that in stm32.
