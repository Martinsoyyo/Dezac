...
#include "App.hpp"
#include "Autogenerated_PostProcess.hpp"

void PostProcess::Do_Post_Process(std::string_view filename)
{
	for (const auto& [file, functions] : postProcessData) {
		if (file == filename) {
			std::cout << "Processing functions for file: " << filename << "\n";
	...
	std::cerr << "Error: No se encontraron funciones para el archivo " << filename << "\n";
}
...
{
	Value& subJSON = const_cast<Value&>(APP::DATAMANAGER.Get_JSON(filename, location));
	if (subJSON.IsString()) {
		std::string old_value = subJSON.GetString();
		if (old_value != new_value) {
			Document& doc = const_cast<Document&>(APP::DATAMANAGER[filename]);
...
bool PostProcess::Change_Language_For_Export_XML()
{
	...
	const std::string& lang = APP::DATAMANAGER.Get_String("Center.json", { "CONFIGURATION", "GENERAL" ,"GENERAL_SETTINGS","Language","Type" });
...
bool PostProcess::Update_Read_Only_Variables_With_Is_Values()
{
	// Helper Lambda.
	auto update_RO_Value = [this](
		...
		{
			auto str = APP::DATAMANAGER.Get_String(from, typeLocation);
	...
	auto max_eq_logic_str = APP::DATAMANAGER.Get_String("Center.json", { "CONFIGURATION", "INFORMATION", "Logical_Equipment" , "Max" });
...
{
	auto max_eq_logic_str = APP::DATAMANAGER.Get_String("Center.json", { "CONFIGURATION", "INFORMATION", "Logical_Equipment" , "Max" });
	auto num_eq_logic_str = APP::DATAMANAGER.Get_String("Center.json", { "CONFIGURATION", "INFORMATION", "Logical_Equipment" , "Value" });
	size_t max_eq_logic = std::stoul(max_eq_logic_str);
	size_t num_eq_logic = std::stoul(num_eq_logic_str);

	using namespace std::placeholders;
	Document& doc = const_cast<Document&>(APP::DATAMANAGER["Center.json"]);
	// These logical devices will be shown.
	for (size_t i = 0; i < num_eq_logic + 1; i++) {
		std::string eq = "P" + std::to_string(i);
		Value& subJSON = const_cast<Value&>(APP::DATAMANAGER.Get_JSON("Center.json", { "CONFIGURATION", "POSITIONS", eq }));
...
bool PostProcess::Check_User_Curve_Names_And_Change_Data_Type()
{
	...
	auto str_num_user_curve = APP::DATAMANAGER.Get_String("Center.json", { "CONFIGURATION", "INFORMATION", "Max_User_Curves." , "Value" });
	size_t num_user_curve = std::stoul(str_num_user_curve);
	for (size_t i = 1; i < num_user_curve + 1; i++) 
	{
		std::string curve_id = "USER_CURVE_" + std::to_string(i);
		auto curve_name = APP::DATAMANAGER.Get_String("LogicCircuit.json", { "CONFIGURATION", "SETTINGS", "USER_CURVE", "INTERPOLATION", curve_id, "Name", "Value" });
	...
	Document& doc = const_cast<Document&>(APP::DATAMANAGER["Datatypes.json"]);
	const Value& Datatypes = APP::DATAMANAGER.Get_JSON("Datatypes.json");
...
bool PostProcess::Show_Or_Hide_Protections_Unit_And_Automatism()
{
	// Helper Lambda.
	auto PresentMissing = [this](
		...
		{
			using namespace std::placeholders;
			Document& doc = const_cast<Document&>(APP::DATAMANAGER["LogicCircuit.json"]);
			Value& destJSON = const_cast<Value&>(APP::DATAMANAGER.Get_JSON(filename, nodeToSet));
			std::string type = APP::DATAMANAGER.Get_String(filename, isPresentLocation);
...
bool PostProcess::Check_If_User_Curve_As_Good_Values_In_Is_Settings()
{
	...
	auto CheckValues = [](
		...
		{
			const Value& subJSON = APP::DATAMANAGER.Get_JSON(filename, Location);
	...
	auto num_user_curve = std::stoul(APP::DATAMANAGER.Get_String("Center.json", { "CONFIGURATION", "INFORMATION", "Max_User_Curves.", "Value" }));
